...
#' @export
#'
scan_mapper2D <- function(f, distance_matrix, filter_values, num_intervals_min, num_intervals_max, num_intervals_steps = 10, percent_overlap_min, percent_overlap_max, percent_overlap_steps = 10, num_bins_when_clustering = 10, num_perms = 1000, seed = 10, num_cores = 1, significance_threshold = 0.05) {
  ...
  worker <- function(m) {
    ...
    for (percen in yy) {
      ...
      pr <- rayleigh_selection(gg, f, num_perms = num_perms)
      th$p <- c(th$p, pr$p)
      th$q <- c(th$q, pr$q)
      th$R <- c(th$R, pr$R)
